...
public class CommandController implements ColorParser, InputValidator, IndexOffset {
	...
	public CommandController(Stage stage, MatchController root, GameComponentsController game,
		...
		Path filePath = Paths.get("readme-resources/CamelQuestionDB.json");
		this.questionManager = new QuestionManager(filePath);
	}
	...
	// Helper method to execute the move logic
	private void executeMove(String fro, String to) {
	    MoveResult moveResult;
	    ...
	    switch (moveResult) {
	        ...
	        case MOVED_TO_HOME_FROM_BAR:
	            ...
	            break;
	        case MOVE_TO_BAR:
	            game.moveToBar(Integer.parseInt(to));
	            soundFXPlayer.playCheckerHitSound();
	            infoPnl.print("Checker moved to bar.");
	            break;
	        case PIP_EMPTY:
	            infoPnl.print("Starting pip has no checkers.", MessageType.ERROR);
	            break;
