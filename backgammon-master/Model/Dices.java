...
public class Dices extends HBox implements ColorParser {
	...
	public DieResults getTotalRoll(DieInstance instance) {
		...
		for (int i = 0; i < numDices; i++) {
			if(instance.equals(DieInstance.DIFF)) {
				int diff = Dice.roll2();
				System.out.println(diff);
				res.add(dices[i].draw(diff));
			}
			else {
				if(LevelController.level.equals("Easy") || LevelController.level.equals("Medium")) {
				res.add(dices[i].draw(dices[i].roll()));
				}
				if(LevelController.level.equals("Hard")) {
					res.add(dices[i].draw(dices[i].rollhard()));
				}
			}
		}
