...
import Control.GameComponentsController;
...
public class Board extends BoardMoves {
	...
	public void unhighlightPipsAndCheckers() {
	    for (int i = 0; i < pips.length; i++) {
	        pips[i].setNormalImage(); // Reset the Pip's image

	        // Check if the Pip is not empty and has a top checker
	        if (!pips[i].isEmpty() && pips[i].getTopChecker() != null) {
	            pips[i].getTopChecker().setNormalImage(); // Reset the top Checker's image
	        }
	    }
	}
	...
	public void highlightFromPipsAndFromBarChecker(Moves moves) {
	    ...
	    if (moves == null) {
	        System.err.println("Error: 'moves' is null. Cannot highlight moves.");
	...
	public DieResults rollDices(PlayerPerspectiveFrom pov) {
	    ...
	    switch (pov) {
	        case BOTTOM:
	            ...
	            if (LevelController.level.equals("Medium") && diffDice != null) {
	                rightBoard.setRight(diffDice); // Display the third dice
	                diffDice.getTotalRoll(DieInstance.DIFF); // Roll for display only
	            }
	            if (LevelController.level.equals("Hard") && diffDice != null) {
	                rightBoard.setRight(diffDice); // Display the third dice
	                diffDice.getTotalRoll(DieInstance.DIFF); // Roll for display only
	            }
	...
	public void redrawDices() {
	    ...
	    // Add additional dice for "Medium" level
	    if (LevelController.level.equals("Medium") && diffDice != null) {
	        // Position diffDice on the right side of the right board
	        rightBoard.setRight(diffDice);
	    }
	    else if (LevelController.level.equals("Hard") && diffDice != null) {
	        rightBoard.setRight(diffDice);

	    }
	    else {
	        // Clear any additional dice if not in "Medium" level
	        rightBoard.setRight(null);
	    }
	}
